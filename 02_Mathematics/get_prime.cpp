...
vector<int> get_prime_list_division(int less_than){
...
int speed_test(int lim){
    ...
    auto t2 = clock();
    cout << "get prime (d) used time: " << (t1 - t0) / double(CLOCKS_PER_SEC) << endl;
    cout << "get prime (f) used time: " << (t2 - t1) / double(CLOCKS_PER_SEC) << endl;
    cout << "two algorithm get the same result." << (prime_division == prime_flag) << endl;
}
