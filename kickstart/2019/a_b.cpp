...
void solve(int _turn) {
    ...
    vector<vector<int>> max_dis(R, vector<int>(C, INT_MIN));
    vector<vector<int>> dp = max_dis;

    rep(i, R) fill(allof(dp[i]), INT_MIN);
    rep(x, R) rep(y, C){
        if(graph[x][y] == '0') dp[x][y] = 0;
    }
    rep(x, R) rep(y, C){
        if(x != 0) dp[x][y] = max(dp[x][y], dp[x - 1][y] + 1);
        if(y != 0) dp[x][y] = max(dp[x][y], dp[x][y - 1] + 1);
        max_dis[x][y] = max(dp[x][y], max_dis[x][y]);
    }
    ...
    repi(x, R) repi(y, C){
        if(x != R - 1) dp[x][y] = max(dp[x][y], dp[x + 1][y] + 1);
        if(y != C - 1) dp[x][y] = max(dp[x][y], dp[x][y + 1] + 1);
        max_dis[x][y] = max(dp[x][y], max_dis[x][y]);
        if(graph[x][y] == '0'){
            min_dis = min(min_dis, max_dis[x][y]);
        }
    }

    printf("Case #%d: %d\n", _turn, min(maxd, min_dis));
}
