...
inline char subtract(char S, char C) {
    return char('A' + (int(S) - int(C) + 26) % 26);
}
...
void solve(int _turn) {
    ...
    if (N % 2) {
        ...
        res[1] = buff[0];
        res[N - 2] = buff[N - 1];
        for (int i = 2, j = N - 3; i < N and j >= 0; i += 2, j -= 2) {
            res[i + 1] = subtract(buff[i], res[i - 1]);
            res[j - 1] = subtract(buff[j], res[j + 1]);
        }
    }
