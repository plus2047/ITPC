...
void solve(int _turn) {
    ...
    rep(i, N) {
        ...
        succ = succ and c1 <= 2 and c2 <= 2;
    }
    
    for (int i = 0, j = 1; j < N and succ; i += 2, j += 2) {
        if (count(allof(mat[i]), 'X') == 1) {
            i--, j--;
        } else {
            succ = succ and mat[i] == mat[j];
        }
    }

    printf("Case #%d: %s\n", _turn, succ ? "POSSIBLE" : "IMPOSSIBLE");
}
