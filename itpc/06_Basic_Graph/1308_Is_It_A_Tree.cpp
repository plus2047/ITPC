...
int main() {
    ...
    for (int _case = 1;; _case++) {
        ...
        int x, y;
        while (scanf("%d%d", &x, &y), x > 0 and y > 0) {
            in_deg[y]++;
            if (in_deg.count(x) == 0) in_deg[x] = 0;
        }
        if (x < 0 or y < 0) break;

        int deg_0 = 0, deg_other = 0;
        for (map<int, int>::iterator iter = in_deg.begin();
             iter != in_deg.end(); iter++) {
            int d = iter->second;
        ...
        bool succ = (deg_0 == 1 and deg_other == 0) or in_deg.empty();
